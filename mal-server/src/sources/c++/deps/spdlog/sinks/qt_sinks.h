...
protected:
  struct invoke_params {
    invoke_params(int max_lines, QTextEdit *q_text_edit, QString payload,
                  QTextCharFormat default_color, QTextCharFormat level_color,
  ...
  // Add colored text to the text edit widget. This method is invoked in the GUI
  // thread. It is a static method to ensure that it is handled correctly even
  ...
  static void invoke_method_(invoke_params params) {
    ...
    // if color range not specified or not not valid, just append the text with
    // default color
    ...
    cursor.insertText(
        params.payload.mid(params.color_range_start,
...
template <typename Factory = spdlog::synchronous_factory>
inline std::shared_ptr<logger>
qt_color_logger_mt(const std::string &logger_name, QTextEdit *qt_text_edit,
...
template <typename Factory = spdlog::synchronous_factory>
inline std::shared_ptr<logger>
qt_color_logger_st(const std::string &logger_name, QTextEdit *qt_text_edit,
                   int max_lines, bool is_utf8 = false) {
